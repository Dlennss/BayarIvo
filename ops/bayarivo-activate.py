#!/usr/bin/python3
"""Activate fixed BayarIvo staging paths; roll back both units if health checks fail."""
import fcntl
import os
...
BASE = Path('/var/lib/syslog-ng/fadlanpulsa/deployment/bayarivo')
STAGE = Path('/var/lib/syslog-ng/bayarivo')
...
with open('/run/lock/bayarivo-deploy.lock', 'w') as lock:
    fcntl.flock(lock, fcntl.LOCK_EX)
    commit = (STAGE / 'frontend/DEPLOY_COMMIT').read_text().strip()
    if not re.fullmatch('[0-9a-f]{40}', commit):
        raise SystemExit('Invalid release commit')
    for name in ['server.js', '.next/BUILD_ID', 'public/deploy-version.txt', 'public/bayarivo-assets/00_full_screen/dashboard_full.png']:
        if not (STAGE / 'frontend' / name).is_file():
            raise SystemExit('Incomplete frontend: ' + name)
    binary = STAGE / 'pulsa-be/bayarivo-api'
    ...
    shutil.copyfile(binary, release / 'bayarivo-api')
    os.chmod(release / 'bayarivo-api', 0o755)
    (frontend / '.next/cache').mkdir(exist_ok=True)
    units = {
        'bayarivo-fe': '[Service]\nWorkingDirectory=' + str(frontend) + '\nBindPaths=\nBindPaths=/var/cache/bayarivo-fe:' + str(frontend / '.next/cache') + '\n',
        'bayarivo-be': '[Service]\nExecStart=\nExecStart=' + str(release / 'bayarivo-api') + '\n',
    }
    previous = {}
    try:
        ...
        run('systemctl', 'restart', 'bayarivo-be', 'bayarivo-fe')
        for attempt in range(30):
            try:
                fetch('http://127.0.0.1:8104/health')
                actual = fetch('http://172.22.0.1:33030/deploy-version.txt').strip()
                fetch('http://172.22.0.1:33030/')
                if actual == commit:
                    print('BayarIvo active: ' + commit, flush=True)
                    break
            except Exception:
                pass
            time.sleep(2)
        else:
            raise RuntimeError('BayarIvo health/release verification failed')
    except Exception:
