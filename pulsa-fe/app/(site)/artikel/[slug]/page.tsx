...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!article) {
    return {
      title: "Artikel Tidak Ditemukan | Bayarivo",
    };
  }

  return {
    metadataBase: new URL(CANONICAL_SITE_URL),
    title: `${article.title} | Bayarivo`,
...
export default async function ArtikelDetailPage({ params }: Props) {
  ...
  const ctaDescription =
    article.category === "Bisnis Agen"
      ? "Setelah memahami arahnya, lanjutkan ke halaman Bayarivo yang paling membantu pertumbuhan usaha dan layanan member Anda."
      : article.category === "Panduan Member"
        ...
        : article.category === "H2H"
          ...
          : article.category === "Deposit"
            ...
            : "Jika Anda sudah memahami produknya, lanjutkan ke halaman layanan Bayarivo yang paling relevan untuk kebutuhan Anda.";
  ...
  return (
    <main className="relative overflow-hidden bg-slate-50 text-slate-900">
      ...
      <article className="relative space-y-4 px-4 pb-6 pt-4">
        ...
        <section className="rounded-md border border-sky-200 bg-linear-to-r from-[#126b9a] via-[#126b9a] to-[#126b9a] p-4 text-white shadow-[0_24px_60px_rgba(15,111,203,0.22)]">
          ...
          <Link
            ...
            className="mt-4 inline-flex h-11 items-center justify-center rounded-full bg-white px-5 text-sm font-semibold text-[#126b9a]! visited:text-[#126b9a]! hover:bg-sky-50 hover:text-[#126b9a]!"
          >
            {article.ctaLabel}
          </Link>
        </section>
