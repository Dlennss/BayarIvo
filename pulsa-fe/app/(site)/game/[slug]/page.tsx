...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "Top Up Game Online | Bayarivo",
      description: "Top up game online favorit Anda di Bayarivo.",
      path: "/game",
    });
  }
  ...
  return buildPageMetadata({
    title: `Top Up ${brandTitle} Murah | Bayarivo`,
    description: lowestPrice
      ? `Top up ${brandTitle} di Bayarivo dengan pilihan nominal aktif. Harga mulai Rp ${lowestPrice.toLocaleString("id-ID")} dan transaksi cepat.`
      : `Top up ${brandTitle} di Bayarivo dengan pilihan nominal yang jelas dan proses transaksi cepat.`,
    path: `/game/${slug}`,
...
export default async function GameBrandPage({ params, searchParams }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `Top Up ${brandTitle} Murah | Bayarivo`,
    description: `Top up ${brandTitle} di Bayarivo dengan pilihan nominal yang jelas dan proses transaksi cepat.`,
    path: `/game/${slug}`,
  ...
  const faqJsonLd = buildFaqJsonLd([
    {
      question: `Apakah top up ${brandTitle} di Bayarivo tersedia dalam banyak pilihan nominal?`,
      answer: `Ya. Bayarivo menampilkan produk ${brandTitle} yang aktif agar pembeli bisa memilih nominal yang sesuai kebutuhan.`,
    },
    {
      question: `Bagaimana cara top up ${brandTitle} di Bayarivo?`,
    ...
    },
    {
      ...
      answer: `Bisa. Halaman ini bisa dipakai pembeli umum maupun member Bayarivo untuk transaksi pribadi dan penjualan ulang.`,
    },
  ]);
  ...
  return (
    <main className="min-h-screen bg-[#eef7f3] pb-24">
      ...
      <div className="space-y-4 px-4 pt-4">
        <section className="overflow-hidden rounded-[26px] border border-emerald-950/5 bg-linear-to-br from-[#163d52] via-[#126b9a] to-[#22c55e] p-4 text-white shadow-[0_18px_42px_rgba(6,78,59,0.22)]">
            ...
            <div className="relative flex items-center gap-3">
              <div className="grid h-16 w-24 shrink-0 place-items-center overflow-hidden rounded-2xl bg-white shadow-[0_14px_30px_rgba(5,46,38,0.20)]">
                {brandLogo?.src ? (
                  ...
                  <Gamepad2 className="h-8 w-8 text-[#126b9a]" />
                )}
              </div>
