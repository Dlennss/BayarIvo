...
function iconToneClass(tone: StatTone) {
  if (tone === "mint") return "bg-white text-[#163d52] ring-2 ring-teal-700";
  if (tone === "lime") return "bg-white text-[#163d52] ring-2 ring-lime-700";
  if (tone === "blue") return "bg-white text-[#163d52] ring-2 ring-sky-700";
  if (tone === "gold") return "bg-white text-[#163d52] ring-2 ring-amber-700";
  return "bg-white text-[#163d52] ring-2 ring-emerald-700";
}
...
export default function AdminHome() {
  ...
  const stats = [
    { title: "Akun Pengguna", value: fmtNumber(data.retailCount), icon: Users, tone: "green" as StatTone, desc: "User dan agent Bayarivo" },
  ...
  return (
    <div className="-m-2 min-h-screen bg-[#f3f7fa] p-3 text-slate-950 sm:p-5 lg:p-6">
        ...
        <section className="overflow-hidden rounded-[30px] border border-emerald-100 bg-white shadow-[0_24px_60px_rgba(6,78,59,0.10)]">
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_330px]">
            <div className="relative overflow-hidden bg-[linear-gradient(135deg,#064e3b_0%,#047857_56%,#72d833_130%)] p-5 text-white sm:p-7">
              ...
              <p className="relative inline-flex items-center gap-2 rounded-full border border-white bg-[#163d52] px-3 py-1 text-[11px] font-black uppercase tracking-[0.22em] text-white">
                <Zap className="h-3.5 w-3.5 fill-white text-white" />
                Super Admin Bayarivo
              </p>
              ...
              <p className="relative mt-3 max-w-2xl text-sm font-semibold leading-6 text-emerald-50">
                Pantau pengguna, kredit agent, transaksi, saldo, dan produk dari satu dashboard operasional Bayarivo.
              </p>
              <div className="relative mt-5 flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-2 rounded-full border border-[#163d52] bg-white px-3 py-1.5 text-xs font-black text-[#163d52]">
                  <BadgeCheck className="h-4 w-4" />
                  Super Admin aktif
                </span>
                <span className="inline-flex items-center gap-2 rounded-full border border-white bg-[#163d52] px-3 py-1.5 text-xs font-black text-white">
            ...
            <div className="flex flex-col justify-center gap-3 bg-[#f8fffb] p-5 sm:p-7">
              <div className="rounded-[24px] border border-emerald-100 bg-white p-4 shadow-[0_14px_30px_rgba(6,78,59,0.06)]">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#163d52]">Kesehatan Sistem</p>
                <p className="mt-2 text-2xl font-black text-slate-950">{loading ? "Memuat..." : "Tersinkron"}</p>
                <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">Data dashboard diambil langsung dari API internal Bayarivo.</p>
              </div>
              <Link
                href="/dashboard/admin/integrasi/pulsa24jam"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border-2 border-[#163d52] bg-white px-4 py-3 text-sm font-black text-[#163d52] shadow-[0_12px_24px_rgba(6,78,59,0.10)] outline-none transition hover:bg-[#f8fffb] focus-visible:ring-4 focus-visible:ring-emerald-200"
              >
                Koneksi Pulsa24Jam
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white text-[#163d52] ring-2 ring-[#163d52] transition group-hover:bg-[#163d52] group-hover:text-white">
                    <Icon className="h-5 w-5" />
                  </span>
                </div>
              </Link>
