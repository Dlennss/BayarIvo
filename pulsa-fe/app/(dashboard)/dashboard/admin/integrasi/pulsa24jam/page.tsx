...
const commandRows = [
  ...
  },
  {
    ...
    catatan: "Refid wajib unik dari sistem Bayarivo.",
  },
  {
    command: "INQ",
...
export default function Pulsa24JamIntegrationPage() {
  ...
  const steps = [
    {
      ...
      desc: "Daftarkan IP publik server Bayarivo di Pulsa24Jam. Tanpa ini, transaksi H2H akan ditolak.",
      icon: ShieldCheck,
    },
    {
      title: "2. Webhook Callback",
      desc: "Pasang URL callback Bayarivo agar status pending, sukses, gagal, SN, dan keterangan final masuk otomatis.",
      icon: Webhook,
    },
    {
      title: "3. API Key",
  ...
  return (
    <main className="-m-2 min-h-screen bg-[#f3f7fa] p-3 text-slate-950 sm:p-5 lg:p-7">
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-5">
        <header className="overflow-hidden rounded-[28px] border border-emerald-200 bg-white shadow-[0_24px_60px_rgba(6,78,59,0.10)]">
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="bg-[linear-gradient(135deg,#064e3b_0%,#047857_62%,#55c72f_100%)] px-5 py-7 text-white sm:px-7 lg:px-9">
              <p className="inline-flex items-center gap-2 rounded-full border border-white bg-[#163d52] px-3 py-1 text-[11px] font-black uppercase tracking-[0.22em] text-white">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                <span className="rounded-full border border-[#163d52] bg-white px-3 py-1.5 text-xs font-black text-[#163d52]">
                  Provider: Pulsa24Jam
                </span>
                <span className="rounded-full border border-white bg-[#163d52] px-3 py-1.5 text-xs font-black text-white">
            ...
            <aside className="flex flex-col justify-center gap-4 bg-[#f8fffb] p-5 sm:p-7">
              <div className={isReady ? "rounded-[24px] border-2 border-[#163d52] bg-white p-5" : "rounded-[24px] border-2 border-amber-700 bg-white p-5"}>
                <div className="flex items-center gap-3">
                  <span className={isReady ? "grid h-12 w-12 place-items-center rounded-2xl bg-[#163d52] text-white shadow-[0_12px_24px_rgba(0,168,120,0.20)]" : "grid h-12 w-12 place-items-center rounded-2xl bg-white text-amber-800 ring-2 ring-amber-800 shadow-[0_12px_24px_rgba(251,191,36,0.22)]"}>
                ...
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-200">
                  <div className="h-full rounded-full bg-[#163d52]" style={{ width: `${(readyCount / envItems.length) * 100}%` }} />
                </div>
              ...
              <div key={item.key} className="rounded-[22px] border border-emerald-100 bg-white p-4 shadow-[0_14px_34px_rgba(6,78,59,0.07)]">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#163d52]">{item.label}</p>
                  ...
                  <span className={ok ? "grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-white text-[#163d52] ring-2 ring-[#163d52]" : "grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-white text-rose-700 ring-2 ring-rose-700"}>
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
          <div className="rounded-[28px] border border-emerald-100 bg-white p-5 shadow-[0_20px_46px_rgba(6,78,59,0.08)] sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-black uppercase tracking-[0.20em] text-[#163d52]">Koneksi Transaksi</p>
                  <h2 className="mt-1 text-xl font-black text-slate-950">Jalur hit Bayarivo ke Pulsa24Jam</h2>
                  <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">
                    Frontend dan panel hanya membuat order atau monitoring. Backend Bayarivo yang menyimpan API key, mengirim command, mencatat refid, lalu menerima callback final.
                  </p>
                </div>
              </div>
            ...
            <div className="mt-5 rounded-[22px] border border-dashed border-emerald-300 bg-[#f7fffb] p-4">
              ...
              <p className="mt-2 break-all text-sm font-black text-[#163d52]">{callbackURL}</p>
            </div>
            <div className="mt-3 rounded-[22px] border border-dashed border-emerald-300 bg-white p-4">
              ...
              <p className="mt-2 break-all text-sm font-black text-[#163d52]">{trxURL}</p>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <Link href="/dashboard/admin/master/produk/provider/map" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border-2 border-[#163d52] bg-white px-4 py-3 text-sm font-black text-[#163d52] shadow-[0_12px_24px_rgba(6,78,59,0.10)] outline-none transition hover:bg-[#f8fffb] focus-visible:ring-4 focus-visible:ring-emerald-200">
                Buka Mapping
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href="/dashboard/admin/transaksi/provider" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border-2 border-[#163d52] bg-white px-4 py-3 text-sm font-black text-[#163d52] outline-none transition hover:bg-slate-50 focus-visible:ring-4 focus-visible:ring-emerald-200">
                Log Provider
          ...
          <div className="rounded-[28px] border border-emerald-100 bg-white p-5 shadow-[0_20px_46px_rgba(6,78,59,0.08)] sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.20em] text-[#163d52]">Setup Live</p>
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <div key={step.title} className="flex gap-3 rounded-[20px] border border-slate-100 bg-[#fbfffd] p-3">
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#163d52] text-white">
        ...
        <section className="rounded-[28px] border border-emerald-100 bg-white p-5 shadow-[0_20px_46px_rgba(6,78,59,0.08)] sm:p-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.20em] text-[#163d52]">Command H2H</p>
            ...
            <span className="w-fit rounded-full bg-[#163d52] px-3 py-1.5 text-xs font-black text-white">
              Jangan taruh API key di frontend
            </span>
          </div>

          <div className="mt-5 overflow-hidden rounded-[22px] border border-slate-200">
            <div className="hidden grid-cols-[120px_160px_minmax(0,1fr)_220px_minmax(0,1fr)] gap-3 bg-[#163d52] px-4 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-white md:grid">
              <span>Command</span>
              <span>Panel</span>
              <span>Fungsi</span>
            ...
            <div className="divide-y divide-slate-100">
              {commandRows.map((row) => (
                <div key={row.command} className="grid gap-3 px-4 py-4 text-sm md:grid-cols-[120px_160px_minmax(0,1fr)_220px_minmax(0,1fr)] md:items-center">
                  <div>
                    <span className="inline-flex rounded-xl bg-emerald-50 px-3 py-1 text-xs font-black text-[#126b9a] ring-1 ring-emerald-100">
                      {row.command}
                    </span>
                  </div>
