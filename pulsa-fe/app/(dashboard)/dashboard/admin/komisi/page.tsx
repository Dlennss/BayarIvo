...
export default function AdminBusinessReportPage() {
  ...
  return (
    <div className="-m-2 min-h-screen bg-[#f3f7fa] p-3 text-[#071b14] sm:p-5 lg:p-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ...
          >
            <div className="text-xs font-black uppercase tracking-[0.12em] text-[#163d52]">{item.label}</div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-[1480px] w-full text-sm">
                    <thead className="bg-[#f1fff8] text-left">
                      <tr>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Tanggal</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Jumlah Trx</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Nominal Trx</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Total Bayar Provider</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Margin</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Komisi Dibayar</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Pengeluaran Transaksi</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Deposit Member</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Deposit Provider Bank</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Selisih Deposit</th>
                        <th className="px-4 py-3 text-xs font-black uppercase tracking-wide text-[#163d52]">Keuntungan Bersih</th>
                      </tr>
                    </thead>
                    <tbody>
                      {group.rows.map((row) => (
                        <tr key={`${group.monthKey}-${row.day}`} className="border-t border-emerald-100 odd:bg-white even:bg-[#fbfffd]">
                          ...
                          <td className="px-4 py-3 font-bold text-[#163d52]">{fmtIDR(Number(row.provider_payment_amount || 0))}</td>
                          <td className="px-4 py-3 font-bold text-[#126b9a]">{fmtIDR(Number(row.margin_amount || 0))}</td>
                          ...
                          <td className="px-4 py-3 font-bold text-[#163d52]">{fmtIDR(Number(row.provider_deposit_amount || 0))}</td>
                          <td className="px-4 py-3 font-bold text-[#315847]">{fmtIDR(Number(row.deposit_gap_amount || 0))}</td>
                          <td className="px-4 py-3 font-black text-[#126b9a]">{fmtIDR(Number(row.profit_amount || 0))}</td>
                        </tr>
                      ))}
                      <tr className="border-t-2 border-[#163d52] bg-[#e8fff4]">
                        <td className="px-4 py-3 font-black text-[#163d52]">Total {group.monthKey}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtID(group.totalTransactions)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalAmount)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalProviderPayment)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalMargin)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalCommission)}</td>
                        <td className="px-4 py-3 font-black text-rose-700">{fmtIDR(group.totalTransactionExpense)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalMemberDeposit)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalProviderDeposit)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalDepositGap)}</td>
                        <td className="px-4 py-3 font-black text-[#163d52]">{fmtIDR(group.totalProfit)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
