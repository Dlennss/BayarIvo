...
export default function Header() {
  ...
  return (
    <header className="brand-app-header sticky top-0 z-50 overflow-hidden bg-[#163d52] text-white shadow-[0_16px_34px_rgba(5,46,38,0.22)]">
      ...
      <div className="mx-auto flex h-18 w-full max-w-7xl items-center justify-between gap-3 px-4">
        <Link href="/" className="flex items-center gap-3" aria-label="Bayarivo">
        ...
        <div className="ml-auto hidden items-center gap-4 lg:flex">
          ...
          <Link href="/login" className="inline-flex items-center gap-2 rounded-2xl bg-linear-to-r from-[#f4ce73] to-[#22c55e] px-6 py-2 text-sm font-black shadow-[0_10px_22px_rgba(163,230,53,0.24)] transition hover:brightness-105">
            <Zap className="h-4 w-4 fill-[#163d52] text-[#163d52]" />
            <span className="text-[#163d52]">Masuk</span>
          </Link>
        </div>
      ...
      {open ? (
        <nav className="space-y-2 border-t border-white/10 bg-[#163d52] px-4 py-4 lg:hidden">
          {navItems.map((item) => (
            <Link
              key={item.label}
              href={item.href}
          ...
          <div className="pt-2">
            <Link
              href="/login"
              className="block rounded-2xl bg-linear-to-r from-[#f4ce73] to-[#22c55e] px-3 py-2 text-center text-sm font-black text-[#163d52]"
              onClick={() => setOpen(false)}
            >
              Masuk
            </Link>
