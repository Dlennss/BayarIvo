...
function buildAgentSummaries(applications: AgentCreditApplication[]): AgentSummary[] {
  ...
  return Array.from(grouped.entries()).map(([memberId, items]) => {
    ...
    return {
      memberId,
      name: applicantText(latest, "agent_name", latest.member_name || latest.agent_name || "Agent Bayarivo"),
                              ...
                              {mode === "operator" ? (
                                <>
                                  ...
                                  <button
                                    ...
                                    className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#126b9a,#84cc16)] text-xs font-black text-white shadow-[0_12px_24px_rgba(4,120,87,0.18)] disabled:opacity-60"
                                  >
