    ...
    <div className="fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-sm md:hidden" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-white/25 bg-white text-[#163d52]"
            onClick={onClose}
                ...
                {section.title ? (
                  <button
                    ...
                    className={`mb-3 flex w-full items-center justify-between rounded-2xl border px-3 py-3 text-left text-[13px] font-black uppercase tracking-[0.10em] outline-none transition focus-visible:ring-4 focus-visible:ring-emerald-200 ${
                      active
                        ? "border-white bg-white text-[#163d52] shadow-[0_14px_28px_rgba(0,0,0,0.16)]"
                  ...
                  >
                    ...
                    <ChevronDown className={`h-4 w-4 transition ${isOpen ? "rotate-180 text-[#163d52]" : "text-lime-100"}`} />
                  </button>
                ) : null}
                {isOpen ? (
