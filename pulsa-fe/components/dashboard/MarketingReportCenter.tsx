...
function agentName(item: AgentCreditApplication) {
  return applicantValue(item, "agent_name", item.member_name || item.agent_name || "Agent Bayarivo");
}
...
export function MarketingReportCenter({ applications }: Props) {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text(`Bayarivo - ${reportTitle}`, 32, 34);
      doc.setFontSize(9);
  ...
  function printReport() {
    ...
    printWindow.document.write(`<!doctype html><html><head><title>${escapeHtml(reportTitle)}</title><style>body{font-family:Arial,sans-serif;color:#111827;margin:28px}header{border-bottom:3px solid #126b9a;padding-bottom:12px;margin-bottom:18px}h1{font-size:22px;margin:0;color:#065f46}h2{font-size:15px;color:#065f46;margin:0 0 4px}p{font-size:11px;margin:5px 0;color:#475569}table{width:100%;border-collapse:collapse;font-size:9px}th{background:#126b9a;color:#fff;text-align:left}th,td{border:1px solid #cbd5e1;padding:6px;vertical-align:top}.approval{margin-top:16px;break-inside:avoid;page-break-inside:avoid}.signatures{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-top:14px;text-align:center}.signature-card{border:1px solid #cbd5e1;border-radius:6px;padding:10px}.signature-card strong{color:#126b9a;font-size:11px}.signature-image{height:58px;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:9px}.signature-image img{max-width:120px;max-height:52px;object-fit:contain}.line{border-top:1px solid #334155;padding-top:5px;font-size:10px;font-weight:700}.signature-card small{display:block;margin-top:3px;color:#64748b;font-size:8px}@page{size:landscape;margin:14mm}</style></head><body><header><h1>Bayarivo - ${escapeHtml(reportTitle)}</h1><p>Periode: ${escapeHtml(periodLabel)} | Dibuat: ${escapeHtml(new Date().toLocaleString("id-ID"))}</p><p>Total data: ${rows.length} | Total pengajuan: ${escapeHtml(formatIDR(totalRequested))} | Total tagihan: ${escapeHtml(formatIDR(totalOutstanding))}</p></header><table><thead><tr>${headers.map((header) => `<th>${escapeHtml(header)}</th>`).join("")}</tr></thead><tbody>${rows.map((row) => `<tr>${headers.map((header) => `<td>${escapeHtml(row[header] ?? "-")}</td>`).join("")}</tr>`).join("")}</tbody></table>${approvalSection}<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  return (
    <section className="mx-auto w-full max-w-7xl overflow-hidden rounded-lg border border-emerald-200 bg-white shadow-[0_18px_42px_rgba(6,78,59,0.08)]">
      <header className="grid gap-4 bg-[linear-gradient(135deg,#163d52,#126b9a)] px-4 py-5 text-white sm:px-6 sm:py-6">
