...
export function MarketingAgentCreditCreateForm({ defaultOpen = false }: MarketingAgentCreditCreateFormProps) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-emerald-200 bg-white shadow-[0_18px_42px_rgba(6,78,59,0.08)]">
      <header className="flex flex-col gap-4 bg-[linear-gradient(135deg,#163d52,#126b9a)] px-4 py-5 text-white sm:flex-row sm:items-center sm:justify-between sm:px-6">
      ...
      <div className="p-3 sm:p-4">
        ...
        {open ? (
          <form onSubmit={submit} className="mt-4 space-y-4">
            ...
            <button type="submit" disabled={busy || loadingAgents} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[linear-gradient(135deg,#126b9a,#65a30d)] text-sm font-black text-white shadow-[0_14px_28px_rgba(5,150,105,0.20)] disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{busy ? "Menyimpan Pengajuan..." : "Simpan Pengajuan & Dokumen"}</button>
          </form>
        ) : null}
      </div>
    </section>
