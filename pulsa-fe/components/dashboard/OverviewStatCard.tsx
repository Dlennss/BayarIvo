...
function toneClass(tone: OverviewStatCardProps["tone"]): string {
  if (tone === "emerald") {
    return "border-emerald-200 border-l-[#126b9a] bg-[linear-gradient(135deg,#ffffff_0%,#f1fff8_100%)]";
  }
  if (tone === "amber") {
...
function iconClass(tone: OverviewStatCardProps["tone"]): string {
  if (tone === "emerald") return "bg-[#e8fff4] text-[#163d52] ring-emerald-300";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs font-black uppercase tracking-[0.12em] text-[#163d52]">{title}</div>
