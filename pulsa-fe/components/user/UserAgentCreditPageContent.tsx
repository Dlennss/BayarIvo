    ...
    <label className={`block ${className}`}>
      ...
      {textarea ? (
        <textarea
          ...
          rows={4}
          className="mt-2 w-full resize-none rounded-2xl border border-slate-200 bg-[#fbfffd] px-4 py-3 text-sm font-bold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-[#126b9a] focus:ring-4 focus:ring-emerald-100"
        />
      ) : (
        <input
          name={name}
          defaultValue={defaultValue}
          placeholder={placeholder}
          className="mt-2 h-12 w-full rounded-2xl border border-slate-200 bg-[#fbfffd] px-4 text-sm font-bold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-[#126b9a] focus:ring-4 focus:ring-emerald-100"
        />
      )}
    </label>
  );
  ...
  return (
    <label className={`block ${className}`}>
      ...
      <select
        name={name}
        defaultValue={defaultValue}
        className="mt-2 h-12 w-full rounded-2xl border border-slate-200 bg-[#fbfffd] px-4 text-sm font-bold text-slate-950 outline-none transition focus:border-[#126b9a] focus:ring-4 focus:ring-emerald-100"
      >
...
function SignaturePad({ signerName, onSignatureChange }: { signerName: string; onSignatureChange: (ready: boolean, dataUrl: string) => void }) {
  ...
  return (
    <div className="rounded-[24px] border border-emerald-300 bg-[linear-gradient(180deg,#ffffff_0%,#f3fff9_100%)] p-4 text-center shadow-[0_12px_26px_rgba(4,120,87,0.10)]">
      ...
      <p className={hasSignature ? "mt-1 text-[9px] font-black text-[#126b9a]" : "mt-1 text-[9px] font-black text-slate-400"}>
...
export function UserAgentCreditPageContent({ name, email, phone, storeName = "", mainBalance = 0, initialApplications = [] }: UserAgentCreditPageContentProps) {
  ...
  const statusBandSubcopy = statusIsRejected
    ...
    : statusIsWaiting
      ? "Menunggu keputusan tim Bayarivo"
      : statusIsPaid
  ...
  const paymentMethods = [
    {
      ...
      title: "Transfer Bank",
      desc: "BCA - 1234567890 a.n. Bayarivo",
      icon: Landmark,
    },
    {
      id: "qris" as const,
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,#052e26_0%,#047857_58%,#84cc16_140%)] px-4 pb-7 pt-5 text-white shadow-[0_18px_42px_rgba(4,120,87,0.22)]">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="flex h-12 shrink-0 items-center gap-2 rounded-2xl bg-white px-2.5 text-[#126b9a] shadow-[0_10px_22px_rgba(0,0,0,0.08)]"
          ...
          >
            ...
            <span className="hidden min-w-0 text-left min-[380px]:block">
              ...
              <span className="block max-w-[72px] truncate text-[10px] font-black leading-3 text-[#126b9a]">{levelSubtitle}</span>
            </span>
          </Link>
        </div>
      </section>
      ...
      <div className="-mt-4 space-y-4 px-3">
        <section className="overflow-hidden rounded-[28px] border border-emerald-100 bg-white shadow-[0_20px_48px_rgba(6,78,59,0.12)]">
          <div className="flex items-center gap-3 border-b border-emerald-50 p-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-base font-black text-slate-950">{name || "Agent Bayarivo"}</p>
        ...
        <section className="grid grid-cols-2 gap-2 rounded-[24px] border border-emerald-100 bg-emerald-50 p-2 shadow-[0_14px_30px_rgba(6,78,59,0.08)]">
          <button
            ...
            className={activeCreditTab === "list"
              ? "flex min-h-14 items-center gap-3 rounded-[20px] bg-white px-3 text-left text-[#126b9a] shadow-[0_12px_22px_rgba(6,78,59,0.10)]"
        ...
        {activeCreditTab === "list" ? (
          <section className="rounded-[26px] border border-emerald-100 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.10)]">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#126b9a]">Data Pengajuan Agent</p>
            ...
            <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
              {["Semua", "Menunggu", "Diterima", "Ditolak"].map((filter, index) => (
                <span
                  key={filter}
                  className={index === 0
                    ? "shrink-0 rounded-full border border-[#126b9a] bg-white px-3 py-2 text-[10px] font-black text-[#126b9a]"
            ...
            <div className="mt-4 space-y-3">
              {listApplications.length ? (
                listApplications.map((item, index) => {
                  ...
                  const agentName = String(data.agent_name || data.nama_lengkap || `Agent Bayarivo ${index + 1}`);
                  ...
                  return (
                    <div key={item.id || index} className="flex items-center gap-3 rounded-[22px] border border-emerald-100 bg-white p-3 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs font-black text-[#126b9a]">{formatIDR(Number(item.approved_amount || item.requested_amount || 0))}</p>
                      </div>
                      <div className="flex shrink-0 flex-col items-end gap-2">
                        ...
                        <button
                          ...
                          className="rounded-full border border-emerald-100 bg-emerald-50 px-3 py-2 text-[10px] font-black text-[#126b9a]"
                        >
                ...
                <div className="rounded-[24px] border border-dashed border-emerald-200 bg-emerald-50/40 px-4 py-10 text-center">
                  <UserRound className="mx-auto h-9 w-9 text-[#126b9a]" strokeWidth={2.2} />
          ...
          <section className="space-y-4">
            <div className="relative overflow-hidden rounded-[28px] border border-emerald-200 bg-[linear-gradient(135deg,#f0fdf4_0%,#ffffff_72%)] p-5 shadow-[0_18px_42px_rgba(6,78,59,0.10)]">
              ...
              <div className="relative flex items-start justify-between gap-3">
                <span className={statusIsRejected ? "grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-rose-50 text-rose-600 ring-1 ring-rose-100" : "grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-emerald-100 text-[#126b9a] ring-1 ring-emerald-200"}>
                  {statusIsRejected ? <XCircle className="h-7 w-7" strokeWidth={2.5} /> : <BadgeCheck className="h-7 w-7" strokeWidth={2.5} />}
                </span>
                <button
                  type="button"
                  onClick={() => setActiveCreditTab("list")}
                  className="rounded-full border border-emerald-100 bg-white px-3 py-2 text-[10px] font-black text-[#126b9a] shadow-[0_10px_22px_rgba(6,78,59,0.08)]"
                >
                  Kembali
                </button>
              </div>

              <p className={statusIsRejected ? "relative mt-5 text-[10px] font-black uppercase tracking-[0.18em] text-rose-600" : "relative mt-5 text-[10px] font-black uppercase tracking-[0.18em] text-[#126b9a]"}>
              ...
              <div className="relative mt-4 space-y-3">
                {statusSteps.map((step) => (
                  <div key={step.label} className={step.done ? "flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 px-3 py-3" : "flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3"}>
                    <span className={step.done ? "grid h-5 w-5 shrink-0 place-items-center rounded-full border border-[#126b9a] bg-white text-[#126b9a]" : "grid h-5 w-5 shrink-0 place-items-center rounded-full border border-slate-300 bg-white text-slate-400"}>
            ...
            {statusIsApproved && !isCreditSuspended ? (
              <div className="rounded-[28px] border border-emerald-100 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.10)]">
                <div className="flex items-start gap-3">
                  <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-emerald-50 text-[#126b9a]">
                ...
                <div className="mt-4 rounded-2xl bg-[linear-gradient(135deg,#163d52,#126b9a,#16a34a)] px-4 py-3 text-white">
                ...
                <div className="mt-3 rounded-[22px] border border-slate-200 bg-[#fbfffd] p-4">
                  ...
                  <button
                    ...
                    className="mt-3 h-11 w-full rounded-2xl bg-[linear-gradient(135deg,#126b9a,#16a34a)] text-xs font-black text-white shadow-[0_12px_24px_rgba(4,120,87,0.18)]"
                  >
                    Bayar Sebagian
                  </button>
                </div>
        ...
        <section className="rounded-[26px] border border-emerald-950/5 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.10)]">
          <div className="mb-4 flex items-start gap-3">
            <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-lime-50 text-[#126b9a]">
        ...
        <section className="relative isolate overflow-hidden rounded-[30px] border border-emerald-100 bg-white shadow-[0_22px_54px_rgba(6,78,59,0.12)]">
          ...
          <div className="relative p-4">
            <div className="flex items-start gap-3">
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-[22px] bg-[linear-gradient(135deg,#ecfdf5,#d9f99d)] text-[#126b9a] ring-1 ring-emerald-100 shadow-[0_12px_24px_rgba(4,120,87,0.10)]">
                ...
                return (
                  <label key={item.title} className={`${selected ? "border-emerald-300 bg-emerald-50" : "border-dashed border-emerald-200 bg-[linear-gradient(135deg,#f0fdf4,#ffffff)]"} min-w-0 rounded-[22px] border p-3 shadow-[0_10px_22px_rgba(6,78,59,0.055)] ${isRequiredForRevision ? "cursor-pointer" : "cursor-not-allowed opacity-50"}`}>
                    <div className="flex items-start justify-between gap-2">
                      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-white text-[#126b9a] ring-1 ring-emerald-100">
        ...
        <section className="rounded-[26px] border border-emerald-950/5 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.10)]">
          ...
          <ol className="space-y-2 text-[11px] font-semibold leading-5 text-slate-600">
            <li>1. Pinjaman saldo hanya digunakan untuk kebutuhan transaksi operasional di Bayarivo.</li>
            ...
            <li>6. Bayarivo berhak menolak, menunda, atau mengevaluasi ulang pengajuan jika data/bukti tidak sesuai.</li>
          </ol>
          <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-3">
            <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded border border-[#126b9a] bg-white text-[#126b9a]">
            ...
            <span className="text-[11px] font-black leading-4 text-slate-950">Saya sudah membaca, memahami, dan menyetujui syarat & ketentuan kredit saldo Bayarivo.</span>
          </label>
        </section>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="sticky bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-20 flex h-14 w-full items-center justify-center gap-2 rounded-[22px] bg-[linear-gradient(135deg,#163d52,#126b9a,#84cc16)] text-sm font-black text-white shadow-[0_18px_36px_rgba(4,120,87,0.24)] transition disabled:opacity-50"
        >
        ...
        <p className="pb-2 text-center text-[10px] font-semibold text-[#126b9a]">Dokumen hanya digunakan untuk verifikasi pengajuan agent Bayarivo.</p>
          </>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/64 p-4 backdrop-blur-sm sm:p-6">
          <div className="max-h-[calc(100dvh-2rem)] w-full max-w-[520px] overflow-y-auto overscroll-contain rounded-[26px] bg-white p-4 shadow-[0_28px_70px_rgba(15,23,42,0.34)] sm:max-h-[calc(100dvh-3rem)] sm:p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#126b9a]">Kredit Berputar</p>
            ...
            <div className="mt-4 rounded-[22px] bg-[linear-gradient(135deg,#163d52,#126b9a,#16a34a)] p-4 text-white shadow-[0_16px_34px_rgba(4,120,87,0.20)]">
            ...
            <label className="mt-3 block">
              ...
              <input
                ...
                className="h-12 w-full rounded-[18px] border border-slate-200 bg-white px-4 text-sm font-black text-slate-900 outline-none transition focus:border-[#126b9a] focus:ring-2 focus:ring-emerald-100"
              />
            </label>
              ...
              {paymentMethods.map((method) => {
                ...
                return (
                  <button
                    ...
                    className={selected
                      ? "flex min-h-[58px] w-full items-center gap-3 rounded-[18px] border border-[#126b9a] bg-emerald-50 px-3 text-left shadow-[0_10px_22px_rgba(4,120,87,0.08)]"
                  ...
                  >
                    <span className={selected ? "grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white text-[#126b9a]" : "grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-emerald-50 text-[#126b9a]"}>
                    ...
                    {selected ? <Check className="h-4 w-4 shrink-0 text-[#126b9a]" strokeWidth={3} /> : null}
                  </button>
                );
              })}
            </div>
            ...
            <label className="mt-3 flex min-h-[54px] cursor-pointer items-center gap-3 rounded-[18px] border border-dashed border-emerald-200 bg-emerald-50/30 px-3 transition hover:bg-emerald-50">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-white text-[#126b9a]">
            ...
            {paymentSuccess ? (
              <div className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-center text-[11px] font-black text-[#126b9a]">
                {paymentSuccess}
              </div>
            ) : null}
            ...
            <button
              ...
              className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-[18px] bg-[linear-gradient(135deg,#126b9a,#16a34a,#84cc16)] text-xs font-black text-white shadow-[0_14px_28px_rgba(4,120,87,0.18)] transition disabled:opacity-60"
            >
              {paymentSubmitting ? (
                <>
