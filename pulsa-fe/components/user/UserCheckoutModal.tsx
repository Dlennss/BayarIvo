...
export function UserCheckoutModal({
  ...
  function handleDownloadProof() {
    void (async () => {
      ...
      doc.setFontSize(11);
      doc.text("Bayarivo - QRIS Payment Receipt", centerX, 61, { align: "center" });
      doc.setFontSize(10);
      ...
      doc.setFont("helvetica", "normal");
      doc.text("Dokumen ini dibuat otomatis oleh sistem Bayarivo dan valid sebagai bukti pembayaran.", 56, finalY + 58);
            ...
            <div className={`grid gap-3 ${showQRCode ? "grid-cols-2" : "grid-cols-1"}`}>
              {showQRCode ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex h-11 w-full items-center justify-center rounded-2xl bg-linear-to-r from-[#126b9a] to-[#126b9a] px-4 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(15,111,203,0.24)]"
                >
                  Buka QRIS
                </a>
              ) : null}
          ...
          <form id="guest-checkout-form" onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            {needsTopup ? (
              <button
                ...
                className="inline-flex h-13 w-full items-center justify-center rounded-2xl bg-[#126b9a] px-5 text-xs font-black text-white shadow-[0_10px_20px_rgba(4,120,87,0.22)] transition hover:bg-[#036b4d]"
              >
                Isi Saldo
              </button>
            ) : (
              <button
                type="submit"
                disabled={loading}
                className="inline-flex h-13 w-full items-center justify-center rounded-2xl bg-[#163d52] px-11 text-xs font-black text-white shadow-[0_10px_20px_rgba(5,46,38,0.18)] transition group-hover:bg-[#126b9a]"
              >
