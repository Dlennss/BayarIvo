...
export function UserPulsaDataExplorer({
  ...
  return (
    <div className="pb-24">
      <section className="relative overflow-hidden rounded-b-[30px] bg-linear-to-br from-[#163d52] via-[#126b9a] to-[#22c55e] px-4 pb-7 pt-4 text-white shadow-[0_18px_42px_rgba(6,78,59,0.20)]">
        ...
        <div className="relative mt-4 rounded-[22px] bg-white p-2 text-slate-900 shadow-[0_16px_34px_rgba(5,46,38,0.18)]">
          <div className="flex items-center gap-2">
            <div className={`grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-2xl ${detectedOperator ? "bg-white ring-1 ring-slate-200" : "bg-emerald-50 text-[#126b9a]"}`}>
          ...
          {detectedOperator ? (
            <div className="mt-2 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-[#126b9a]">
        ...
        {showTabs ? (
          <div className="relative mt-4 grid grid-cols-2 gap-1 rounded-2xl bg-[#163d52]/28 p-1 ring-1 ring-white/15">
            {(["pulsa", "data"] as TabKey[]).map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex h-10 items-center justify-center gap-2 rounded-xl text-sm font-black transition ${active ? "bg-white text-[#126b9a] shadow-[0_10px_20px_rgba(5,46,38,0.16)]" : "text-white/80"} ${!enabled ? "cursor-not-allowed opacity-40" : ""}`}
                >
