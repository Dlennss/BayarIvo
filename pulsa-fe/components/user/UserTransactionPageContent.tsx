  ...
  return (
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-[11px] font-semibold text-slate-500">Pantau semua transaksi Bayarivo</p>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {RANGE_FILTERS.map((label) => (
          <button
            ...
            className={
              selectedRange === label
                ? "h-9 shrink-0 rounded-full bg-[#126b9a] px-4 text-xs font-black text-white shadow-[0_10px_20px_rgba(4,120,87,0.20)]"
                : "h-9 shrink-0 rounded-full border border-slate-200 bg-white px-4 text-xs font-black text-slate-500 transition hover:border-emerald-200 hover:text-[#126b9a]"
            }
          >
            {label}
          </button>
      ...
      <label className="relative flex h-13 cursor-pointer items-center gap-3 rounded-[16px] border border-slate-200 bg-white px-4 text-slate-500 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
        <CalendarDays className="h-4.5 w-4.5 text-[#126b9a]" strokeWidth={2.1} />
