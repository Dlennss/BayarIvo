...
const paymentMethods = [
  { id: "transfer", title: "Transfer Bank", desc: "BCA - 1234567890 a.n. Bayarivo", icon: Banknote },
...
function getPaymentHelper(methodId: string, applicationId: number) {
  if (methodId === "qris") {
    return {
      ...
      desc: "Scan QRIS dari kanal Bayarivo, lalu upload bukti pembayaran.",
      rows: [
...
function getApplicantLabel(item: AgentCreditApplication) {
  ...
  return {
    name: String(data.agent_name || data.nama_lengkap || "Agent Bayarivo"),
...
export function UserCreditBillPayLaterContent({ bills }: Props) {
  ...
  async function copyPaymentValue() {
    if (!paymentHelper.value || paymentHelper.value === "Saldo akun Bayarivo") return;
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-4">
      <section className="relative overflow-hidden rounded-[30px] bg-[linear-gradient(135deg,#163d52,#047857_58%,#84cc16_135%)] p-5 text-white shadow-[0_22px_50px_rgba(4,120,87,0.22)]">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-[#126b9a]">
      ...
      {bills.length ? (
        <>
          <section className="rounded-[28px] border border-emerald-100 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.08)]">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="rounded-full bg-lime-100 px-3 py-1 text-[10px] font-black text-[#126b9a]">{displayedBills.length} data</span>
            </div>
            ...
            <div className="space-y-2">
              {displayedBills.length ? displayedBills.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={selected ? "grid h-12 w-12 shrink-0 place-items-center rounded-[19px] bg-[#126b9a] text-white shadow-[0_10px_20px_rgba(4,120,87,0.20)]" : "grid h-12 w-12 shrink-0 place-items-center rounded-[19px] bg-slate-100 text-slate-500"}>
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-1 block text-sm font-black text-[#126b9a]">{formatIDR(outstanding)}</span>
                    ...
                    <span className="flex shrink-0 flex-col items-end gap-2">
                      ...
                      {outstanding > 0 ? (
                        <button type="button" onClick={() => selectBill(item.id, true)} className="rounded-full bg-[#126b9a] px-3 py-2 text-[10px] font-black text-white">
                          Bayar
                        </button>
                      ) : null}
                      <Link href="/user/saldo/kredit-agent" className="rounded-full border border-emerald-100 bg-emerald-50 px-3 py-2 text-[10px] font-black text-[#126b9a]">
                        Detail
                      </Link>
                    </span>
                  </div>
            ...
            <section className="overflow-hidden rounded-[28px] border border-emerald-100 bg-white shadow-[0_18px_42px_rgba(6,78,59,0.08)]">
              <div className="bg-[linear-gradient(135deg,#f8fffb,#eefcf4)] p-4">
                <div className="flex items-center gap-3">
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-[#126b9a] ring-1 ring-emerald-100">
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-[linear-gradient(90deg,#126b9a,#84cc16)]" style={{ width: `${usageProgress}%` }} />
                  </div>
                </div>
              </div>
            </section>
            ...
            <div className="mt-3 space-y-2">
              {paymentMethods.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={selected ? "grid h-11 w-11 place-items-center rounded-2xl bg-[#126b9a] text-white" : "grid h-11 w-11 place-items-center rounded-2xl bg-slate-100 text-slate-500"}>
                    ...
                    {selected ? <CheckCircle2 className="h-5 w-5 text-[#126b9a]" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}
                  </button>
                );
              })}
            </div>
          ...
          <section className="rounded-[28px] border border-emerald-100 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.08)]">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-lime-100 text-[#126b9a]">
              ...
              <MethodIcon className="h-5 w-5 text-[#126b9a]" />
            </div>
            <div className="mt-4 space-y-2 rounded-[22px] bg-slate-50 p-3 text-sm">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  {selectedMethod !== "offline" ? (
                    <button
                      ...
                      className="h-9 shrink-0 rounded-full bg-emerald-50 px-3 text-[10px] font-black text-[#126b9a] ring-1 ring-emerald-100"
                    >
                      Salin
                    </button>
                  ) : null}
              ...
              <div className="flex justify-between gap-3">
                ...
                <span className="font-black text-[#126b9a]">Gratis</span>
              </div>
            ...
            <label className="mt-4 flex cursor-pointer items-center gap-3 rounded-[22px] border border-dashed border-emerald-300 bg-emerald-50/60 p-3 transition hover:bg-emerald-50">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white text-[#126b9a] ring-1 ring-emerald-100">
              ...
              <span className={proofFile ? "rounded-full bg-emerald-600 px-3 py-1 text-[10px] font-black text-white" : "rounded-full bg-white px-3 py-1 text-[10px] font-black text-[#126b9a] ring-1 ring-emerald-100"}>
            ...
            <button
              ...
              className="mt-4 flex h-13 w-full items-center justify-center gap-2 rounded-[20px] bg-[linear-gradient(135deg,#163d52,#126b9a,#84cc16)] text-sm font-black text-white shadow-[0_18px_34px_rgba(5,150,105,0.22)] disabled:opacity-50"
            >
        ...
        <section className="grid min-h-[300px] place-items-center rounded-[28px] border border-dashed border-emerald-200 bg-white px-6 text-center shadow-[0_18px_42px_rgba(6,78,59,0.06)]">
          <div>
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-emerald-50 text-[#126b9a]">
