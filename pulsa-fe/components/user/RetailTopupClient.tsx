  ...
  const checkStatus = useCallback(async (refID: string, showLoading = false) => {
    ...
    try {
      ...
      if (item.status === "approved") {
        setMessage("Pembayaran berhasil. Saldo Bayarivo sudah bertambah.");
  ...
  return (
    <div className="space-y-4">
      <section className="px-1">
        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#126b9a]">Isi Saldo</p>
      ...
      {loading ? (
        <div className="grid min-h-48 place-items-center rounded-[28px] bg-white"><LoaderCircle className="h-6 w-6 animate-spin text-[#126b9a]" /></div>
      ) : payment ? (
        <section className="rounded-[28px] border border-emerald-950/5 bg-white p-5 shadow-[0_18px_42px_rgba(6,78,59,0.11)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#126b9a]">Pembayaran QRIS</p>
              <h2 className="mt-1 text-lg font-black text-slate-950">{statusLabel(payment.status)}</h2>
            </div>
            <button type="button" onClick={() => void checkStatus(payment.ref_id, true)} disabled={checking} className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-50 text-[#126b9a] disabled:opacity-60" aria-label="Periksa status pembayaran">
          ...
          {isPending && payment.qr_url ? (
            ...
            <div className="mt-5 grid min-h-44 place-items-center rounded-2xl bg-emerald-50 text-center">
              <div><CheckCircle2 className="mx-auto h-12 w-12 text-[#126b9a]" /><p className="mt-3 text-sm font-black text-emerald-900">Topup berhasil</p></div>
            </div>
          ) : null}
          ...
          <div className="mt-4 space-y-3 rounded-2xl bg-slate-50 p-4">
            ...
            <div className="flex justify-between gap-4 border-t border-slate-200 pt-3 text-sm"><span className="font-black text-slate-700">Total pembayaran</span><strong className="text-[#126b9a]">Rp {fmtIDR(payment.gross_amount || payment.amount)}</strong></div>
            <button type="button" onClick={() => void copy(payment.ref_id)} className="flex w-full items-center justify-between rounded-xl bg-white px-3 py-3 text-left"><span><span className="block text-[9px] font-bold uppercase text-slate-400">Referensi</span><span className="text-xs font-black text-slate-950">{payment.ref_id}</span></span><Copy className="h-4 w-4 text-[#126b9a]" /></button>
          </div>

          {!isPending ? <button type="button" onClick={() => { setPayment(null); setError(""); setMessage(""); void load(); }} className="mt-4 h-12 w-full rounded-2xl bg-[#126b9a] text-xs font-black text-white">Buat Topup Baru</button> : null}
        </section>
      ) : (
        <form className="space-y-4" onSubmit={createPayment}>
          <section className="rounded-[28px] border border-emerald-950/5 bg-white p-4 shadow-[0_18px_42px_rgba(6,78,59,0.11)]">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2.5">{QUICK_AMOUNTS.map((value) => <button key={value} type="button" onClick={() => setAmount(String(value))} className={`h-12 rounded-2xl border text-xs font-black ${nominal === value ? "border-[#126b9a] bg-emerald-50 text-[#126b9a]" : "border-slate-200 bg-slate-50 text-slate-700"}`}>Rp {fmtIDR(value)}</button>)}</div>
            <label className="mt-4 flex h-12 items-center gap-3 rounded-2xl border border-slate-200 bg-[#f8fffb] px-4"><span className="text-xs font-black text-[#126b9a]">Rp</span><input className="min-w-0 flex-1 bg-transparent text-sm font-black text-slate-950 outline-none" inputMode="numeric" placeholder="Minimum 100.000" value={amount ? fmtIDR(nominal) : ""} onChange={(event) => setAmount(event.target.value.replace(/\D/g, ""))} /></label>
          </section>

          <section className="rounded-[28px] border border-[#126b9a] bg-emerald-50 p-4 shadow-[0_18px_42px_rgba(6,78,59,0.08)]">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-[#126b9a]"><QrCode className="h-5 w-5" /></span>
              ...
              <ShieldCheck className="h-5 w-5 text-[#126b9a]" />
            </div>
          </section>

          <button type="submit" disabled={saving} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#126b9a] text-xs font-black text-white disabled:opacity-60">{saving ? <LoaderCircle className="h-4 w-4 animate-spin" /> : null}{saving ? "Membuat QR..." : "Lanjutkan Pembayaran"}<ChevronRight className="h-4 w-4" /></button>
        </form>
      )}

      {history.length ? <section className="rounded-[28px] bg-white p-4"><h2 className="text-sm font-black text-slate-950">Riwayat terbaru</h2><div className="mt-3 space-y-2">{history.slice(0, 5).map((row) => <div key={row.id} className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-3"><div><p className="text-xs font-black text-slate-900">Rp {fmtIDR(row.requested_amount || row.amount)}</p><p className="text-[10px] font-semibold text-slate-500">QRIS</p></div><span className="text-[10px] font-black text-[#126b9a]">{statusLabel(row.status)}</span></div>)}</div></section> : null}
    </div>
  );
}
