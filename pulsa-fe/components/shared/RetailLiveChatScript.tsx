...
export function RetailLiveChatScript() {
  ...
  function submitChat(event: FormEvent<HTMLFormElement>) {
    ...
    const text = [
      "Halo Bayarivo, saya butuh bantuan.",
    ...
    <div className="fixed bottom-[72px] right-3 z-[80] w-[calc(100vw-24px)] max-w-[320px] sm:bottom-20 sm:right-5">
      {open ? (
        <section className="mb-2 overflow-hidden rounded-[18px] border border-lime-200/70 bg-white shadow-[0_22px_54px_rgba(5,46,38,0.20)]">
          <div className="relative overflow-hidden bg-[#163d52] px-3.5 py-3 text-white">
            <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-lime-300/25 blur-2xl" />
            <div className="relative flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-white text-[#126b9a]">
          ...
          <form onSubmit={submitChat} className="space-y-2.5 px-3.5 py-3.5 text-slate-900">
            ...
            <button
              type="submit"
              className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-[#f4ce73] to-[#22c55e] text-xs font-black text-[#163d52] shadow-[0_12px_22px_rgba(34,197,94,0.20)] transition hover:brightness-105 active:scale-[0.98]"
            >
      ...
      <button
        ...
        className="ml-auto flex h-11 w-11 items-center justify-center rounded-2xl bg-[#163d52] text-left text-white shadow-[0_12px_26px_rgba(5,46,38,0.22)] ring-1 ring-lime-300/25 transition hover:translate-y-[-1px] sm:h-12 sm:w-auto sm:justify-start sm:gap-2.5 sm:px-3.5"
        aria-expanded={open}
        aria-label="Chat Bayarivo"
      >
        <span className="grid h-8 w-8 place-items-center rounded-xl bg-linear-to-br from-[#f4ce73] to-[#22c55e] text-[#163d52]">
        ...
        <span className="hidden sm:block">
          ...
          <span className="block text-sm font-black">Chat Bayarivo</span>
        </span>
      </button>
    </div>
  );
